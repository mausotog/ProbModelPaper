...
in APR research: the quality of the generated patches~\cite{kim2013}.  One way to
...
certain mutation operators much more often than others (e.g., adding an $IfStatement$ is \emph{probably} \todo{I guess this word "probably" here means that the probabilities are higher, but it sounds weird in the context} more common than adding a $TypeDeclarationStatement$)~\cite{Soto16}. In this
...
\item \textbf{Empirical model of single-edit repairs to Java code} mined
    ...
    \item \textbf{Development and integration} of this approach for the Java programming language in a tool, available online, which can apply the several different mutation operators, and the probabilistic model discussed in this study.\footnote{We will release the tool post blind-review.} %\footnote{https://bitbucket.org/clegoues/genprog4java}
  \item \textbf{Evaluation} of the probabilistic model in several
    ...
    process benefits from combining mutation operator categories. We also evaluate the quality of the patches found by our approach by comparing them to patches created by current state of the art tools in the same bugs.
...
candidate program statements (typically using off-the-shelf approach, e.g. Tarantula~\cite{Jones02}). 
...
until it reaches a pre-defined time or resource limit. 
...
$replace$) which modify source code. These prior techniques target the C programming language, where
...
method calls. In these approaches, the statements being appended or replaced typically come from within the project being modified. This is grounded in the notion that source code has a high level of redundancy~\cite{gabel10}.
...
\\
\end{tabular}
  \caption{(Top) PAR fix templates. (Middle) PAR extra templates. (Bottom) SPR transformation schemas. \todo{I tried making everything a Figure, but it takes so much more space that way that I made some of them tables}}
...
model of human bug-fixing edits from a large set of popular Java projects on GitHub. The probabilistic model is built from the instance count of the mutation operators and replacements performed historically from developers' bug fixing in popular Github projects. 
...
regular expression to each commit message that looks for words such as $``fix", ``bug", ``issue", ``problem",$
...
\begin{tabular}{lllll}
\hline
Identifier & Project name & Description & Test cases & Bugs considered \\
\hline
...
\newline
\center
  \caption{Description of projects that conform Defects4j \todo{re check the number in the last column}}
  \label{defects4j}
\end{table} 
...
Table~\ref{heldOut} describes the specifics of the held out test suites. We use Randoop~\cite{pacheco07}, an established
...
\centering
\begin{tabular}{llll}
Project & Number of test cases & Line coverage & Branch coverage \\
\hline
...
\end{tabular}
\newline
\caption{Held-out test suite data. These test suites were created using Randoop with a budget of 10 minutes and evaluated coverage using Cobertura.}
\label{heldOut}
\end{table}    
...
\begin{tabular}{l|p{1cm}rr|rrr}
   &\multicolumn{3}{c|}{Replacements} &\multicolumn{3}{c}{Mutations} \\
   	\hline
Test fold	& Instances&Prob&	EqDist&Instances&Prob&	EqDist \\
	\hline
...
\center
  \caption{Correctly predicted percentage according to 10 fold cross
    ...
    equally distribured. The instance count, is the number of actual instances in each fold when running cross validation on each of the different models.} 
  \label{results10fcv}
\end{table} 
...
%  \caption{   }
%\end{table}
Results are shown in Table~\ref{results10fcv}. For each fold, we show the instance count and percentage of correctly 
...
In this section we compare our approach, using all the mutation operations and the probabilistic model, against those patches found in the same benchmark by different state of the art tools.
We evaluate against four different state of the art tools targeting Java: GenProg, TrpAutoRepair, PAR and Nopol.
We extend an available open-source implementation of GenProg adapted for Java to include the behavior and mutation operators of these first three approaches, since they were originally built to target C (GenProg and TRPAutoRepair), or have not made their code available (PAR). We also compare our approach to Nopol\cite{xuanNopol}, an automatic repair tool of conditional statement for Java programs. The authors have created patches for the same benchmark used in this study\cite{martinez2016} and made their results publicly available.\footnote{https://github.com/Spirals-Team/defects4j-repair/tree/master/results/2017-march} For the comparison with Nopol we have used the more up-to-date dataset available in their website marked as ``March 2017".\todo{pending to add more justification depending on Martin's response}
In this study we do not compare directly to SPR/Prophet~\cite{long16proph} mutation operators because they use machine learning to tune the probabilities to pick their schemas and is thus much harder to re implement faithfully.
...
Table~\ref{stateOfTheArtComparison} shows the patches created by our approach, five of them pass the held-out test suite (45.5\%). None of the GenProg, TRPAutorRepair, or PAR patches fully generalize; and 25\% of Nopol's patches are able to generalize. This gives us confidence that our approach produces high quality patches in comparison with the alternatives created by state-of-the-art tools.

One example in which we can very clearly see the success of our approach when applying the probabilistic model is Closure 18 as seen in Figure~\ref{closure18prob} where, not only did our approach created the exact same patch as the human did, but also our approach was able to find the patch within the time frame specified before the execution is shutdown by one of the constraints per execution while other approaches that are able to create this patch, couldn't because they ran out of time. 

This example was fixed by the human by removing an expression from an $IfStatement$ as seen in Figure~\ref{closure18prob}. PAR in particular has a mutation operator specially created to perform this task, and even so, it was not able to find this patch due to the vast search space and the fact that it searches through this space in an equally distributed manner to pick its mutation operators. Nopol was able to create a patch for this example as shown in Figure~\ref{closure18nopol}, but this patch is not able to generalize to a held-out test suite.
...
@  for (CompilerInput input : inputs) {
	...
	\end{lstlisting}
	\caption{A snippet of the source code of the Closure 18 bug fixed using the
      probabilistic model. This patch is identical to what the developer did, and it was able to find the patch while other approaches were not.\label{closure18prob}}
\end{figure}
...
@  for (CompilerInput input : inputs) {
	...
	\end{lstlisting}
	\caption{A snippet of the source code of the Closure 18 bug fixed using Nopol. This patch is not able to generalize to a held-out test suite.\label{closure18nopol}}
\end{figure}
...
\begin{tabular}{l|ll|ll|ll|ll|ll}
 & Probabilistic Model & NFTC & GenProg & NFTC & TRP & NFTC & PAR & NFTC & Nopol  & NFTC  \\
 \hline
Closure 10 & SequenceExchange & 0 & No patch & - & No patch & - & No patch & - & Found & 28   \\
 & SequenceExchange & 0 & - & - & - &-  & - & - & - & -   \\
 \hline
Closure 18 & ExpRemove & 0  & No patch & - & No patch & - & No patch & - &Found &  1  \\
\hline
Math 2 & MethodReplace & 170 & No patch & - & No patch & - & MetthodReplace & 170 & Found & 0  \\
\hline
Time 19 & Append & 0 & Append & 188 & Append & 188 & No patch & - & Found &  188 \\
 & Replace & 0 & - & - & - & - & - & - & - & - \\
\hline
Chart 1 & SequenceExchange & 1 & Replace & 1  & Delete & 1 & ExpAdd & 1 & No patch &   \\
 & Delete & 1 & Delete & 1 & Replace &1  & ExpReplace & 1 & - & -   \\
 & Delete & 1 & Replace & 1 & Delete & 1 &  -& -& - & -  \\
 & SequenceExchange & 1 & Delete & 1 & Delete &1  & - & - & - & -   \\
...
\newline
\center
  \caption{Quality assessment between the probabilistic model and other state of the art approaches. The NFTC column stands for Number of Failed Test Cases, it represents the number of test cases the patch failed in the held-out test suite. If it contains a 0, it means the patch was able to fully generalize to the the held-out test suite}
...
We have released a full set of these replacement rules.\footnote{We will release the full list of replacement rules post blind-review.} %\footnote{\url{https://github.com/mausotog/ReplacementsEmpiricalStudy/blob/master/ResultsAssociationRuleMiningReplacements.txt}} 
...
The authors acknowledge the risk of producing bad quality patches that would not 
generalize
to a different description of the behavior the program should have. We attenuate 
this by 
...
not guarantee that the human created patch is perfect. Nevertheless, we consider 
this to be a
...
But nonetheless, this is a state of the art test suite generation tool that 
provides high quality test suites, in our case, for evaluation purposes.
...
tool and model is based on an instance count of statements rather than an a more 
...
search space at the time of creation, and we are guaranteed that all the 
mutations created are closer to the actual goal.  
...
search space, and our approach is thus less immediately comparable to these 
kinds of approaches.
