...
indicates that by applying the probabilistic model we can find
...
\paragraph{Evaluation of Rules}
To evaluate the effectiveness of the association rules in the context of the automatic program repair process, we first had to remove the section of the corpus where the number of edits performed is less than 3, since the association rules require at least two antecedents and one consequent, therefore by definition the patches with less than three edits can´t be created by the association rules. This removed 62.83\% of the corpus. On the remaining 37.17\% where association rules can be applied. We performed a 10-fold cross validation process as follows:
First we divide our corpus in 10 folds, these folds will be used as testing data.
For each of the folds, we then group the complement of the fold used as testing data (the remaining 9 folds) and from 
the complement we build the association rules using Apriori \todo{citation here of apriori}. We build the rules from
...
We categorize them in three different ways: Fully covered, partially covered, and not covered.
...
\vspace{1pt}
\end{tabular}
  \caption{Instances in the left, rules in the right. \label{rulesandinstances}}
}
\end{table}
...
We performed this process with all the instances and all the rules in each fold and finally, we aggregate the results from all the folds.
...
\begin{tabular}{rrrrr}
\toprule
 Confidence & Number of rules & Fully Cov & Partially Cov & Not Cov \\
\midrule
...
We can see that there is a big threshold in 90\% confidence, where there is a nice balance between having very high number of fully covered instances, while still having very high confidence rules.

\todo{remove this document from the repo https://github.com/mausotog/ReplacementsEmpiricalStudy/blob/master/ResultsAssociationRuleMiningReplacements.txt } 
