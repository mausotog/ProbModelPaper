...
\todo{Suggestion: Rename this to something like ``Generate and validate repair'' or
  ...
  on background in context.  Imperfect, but I think it would be a better organization. RESPONSE: I'm confused about this: We talked about having a background section that can be skipped by people who know the area. The next section is how we categorize the mutation operators, which is a contribution, so people shouldn't skip that. I think they are very different sections that shouldn't be merged.}
...
create candidate patches. These and several other techniques use \todo{this
  statement is still false. RESPONSE: Why is this false? They apply randomness to create variations selecting mut ops from the search space}  randomized 
