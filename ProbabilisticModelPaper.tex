...
schemas~\cite{long16proph,fan15SPR}.  Semantically-informed
...
section of state-of-the-art approaches in two groups: 
...
GitHub to analyze how developers apply different types of edits. The intuition behind this model is to be able to apply human
knowledge to the automatic program repair process and create patches closer to how a human developer would do it. To do this, we select a corpus of popular GitHub projects and filter out their latest bug fixing commits. We then extract an instance count of each of the mutation operators and replacements used in these commits to use them in automatic program repair. Finally we analyze these edits and extract association rules of edits that happen often to be able to use this knowledge for multi-edit source code repairs.
...
\midrule
Fold	& Inst &Prob& EqDist& Inst&Prob& EqDist \\
\midrule
...
Replacements and Mutations models. The ``Inst" columns 
...
Figure~\ref{stateOfTheArtComparison} shows the edit performed and
the number of held-out tests each patched program fails. Of the eleven
...
from having a diverse mutation operator pool,
rather than restricting it to just one category; future improvements in fault localization thus will
