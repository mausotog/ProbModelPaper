...
to constructing candidate patches, and do so either according to a uniform probability
distribution or \todo{Mau can you please try to rephrase this clause?  I'm not
  sure what you mean. Reponse: What I mean is that current GenProg, AE, etc use a probability that was produced by eyeballing the benchmark used and what would work better, not by formally analyzing it, and PAR, etc just use a uniform distribution} a probability adapted from
the results of a particular benchmark.  This may lead to a time increase when searching the fix space to patch bugs, a lack of generalizability when evaluated in a second test suite, and a lack of
understandability and maintainability. In engineering practice, human developers edit source code in several different ways when fixing bugs. Some of these edits are far more common than others (eg, deleting a line of code, is more common than adding a new class), and are also dependent of the context. These edits are represented as mutation operators in an APR context. We 
mined instances of mutation operators in bug-fixing commits from projects to create a probabilistic model describing the distribution of 
likelihood when applying each of the different edits. 
We categorize, compare and evaluate the different mutation operators in 
...
in the past and covers the majority of real source code changes. We evaluated our model in a case study and in 15 bugs from popular open source projects. Our results indicate that by applying the probabilistic model we are able to find the correct mutation operator to patch bugs faster in the majority of bugs studied, while they also indicate that having models with different probabilities to construct the source code variants doesn't increase the likelihood of the created patches to pass a second test suite. 
\end{abstract}
