...
discussion(Section~\cite{discussion}) and threats to validity (Section~\cite{threatsVal}) 
...
\subsection{Example patches} \label{examPatches}
To show an example of the patches being produced by the probabilistic model and the 
equally distributed model we'll show the patches produced in Chart 1 when applying
Statement-Edit mutation operators. In this example we have the code shown in Figure~\ref{fig:chart1.1}
...
...
	\end{lstlisting}
	\caption{Source code of Chart 1. The highlighted section is an if statement that is patched if it is removed.}
	\label{fig:chart1.1}
\end{figure}

While the probabilistic model is much more likely to remove either the whole If statement, or just the body (line 9) as 
shown in Figure~\ref{fig:chart1.2} \todo{I imagine this figure 5 is unnecessary, 
but I'll put it anyway just in case Claire wants it for clarity purposes}, the equally distributed approach is less 
likely to do so and more likely to create patches that are far from what a human developer would have done, and 
what a human developer can understand as detailed in Figure~\ref{fig:chart1.3}.
...
...
	\end{lstlisting}
	\caption{Source code of Chart 1 fixed by the probabilistic model. Removed the if statement.}
	\label{fig:chart1.2}
\end{figure}
...
...
	\end{lstlisting}
	\caption{Source code of Chart 1 fixed by the equally distributed model. Inserted code unlike a developer would have done it and produces undesired behavior.}
	\label{fig:chart1.3}
\end{figure}
