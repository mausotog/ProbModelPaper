...
in order to modify the behavior of a program. In order to analyze the broad 
...
TrpAutoRepair\cite{Qi13} and AE~\cite{Weimer13} in which the main technique for 
creating candidate 
...
apply these mutation operators to statements in the source code in order to 
...
predetermined templates to apply to specific sections of the code in order to 
modify the behavior of the program to make it possibly fix the error in the 
code. In this family we have well known techniques such as PAR~\cite{kim2013}, 
SPR~\cite{fan15}, Kali~\cite{Qi15} and 
...
commonly used changes that human programmers perform in order to fix their 
code~\cite{kim2013}.
...
but aren't mentioned in the paper\cite{kim2013}: 
...
There are other well knownn techniques in this domain, such as the SPR 
approach~\cite{fan15}, which is composed by a set of transformation schemas as 
follows:
\begin{enumerate}
...
For each of the bugs that we were able to create a patch for, we have subtracted the source code of the human patch provided by Defects4j to take this as a description of the behavior of the program after the bug has been fixed, and we have run Randoop for ten minutes (six times the default value, to make sure we have enough test cases to test the validity of the patch) on this version of the code in order to get a different test suite\footnote{Available at: https://goo.gl/YbFeql} than the one used to guide the search. We then ran this second test suite on the patch generated by both models and we obtained the results of Table \ref{tab:singleLineBugs}. The blue values in the right of each slots of the table represent the percentage of number of patches created that passed the second test suite presented and did not overfit to the first set of test cases but generalized to the second independent test suite. 
...
We also notice that there is no difference in the quality of patches when using one approach or the other for these five bugs. Further evaluation should be performed in order to determine if one of the models creates higher quality patches than the other.
