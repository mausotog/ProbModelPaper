...
\center
  \caption{Number of variants it takes to find a patch using different sets of
    ...
    Using Replace. Right) Using Append, Remove and Replace. \todo{Please stop
      using colors and instead use text to label units and meaning.  What is the
...
a patch who would not pass the test cases in the second test suite.  For this evaluation
...
previous work~\cite{legoues12,kim2013}\todo{does Par say the population size
  ...
  genprog as a search strategy (if that's true).  RESPONSE: Yes, PAR uses 40, that is why I mention it in the cite. / Where do I get the generation count information? The config file does not say anything, so it uses the default one.}, we ran 20 seeds per each run, and a timeout of 4 hours since there is 
