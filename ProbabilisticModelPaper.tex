...
programs with minimal human interaction~\cite{legoues12,kim2013,Weimer13,fan15,long15,debroy10,perkins09,wei10}. One well-known class of repair techniques follows a 
...
understand, less efficient and less maintainable~\cite{kim2013}.\todo{the Qi15
  ...
  don't think we evaluate it. RESPONSE: I'm trying to build up the problem by listing a set of problems, from which we will evaluate speed and generalizability} 

Most generate-and-validate approaches build a set of candidate patches by applying mutation operators using a heuristic based or random distribution. In reality, human developers choose some mutation operators 
...
human behavior. Different from them, we take this notion of applying a probabilistic model even earlier in the process, at the time of creating the patch candidates so all the candidates in the pool are built in a way designed to fix the error. We also include a superset of mutation operators and not only the transformation schemas~\cite{fan15}.  

We also categorize, compare and contrast a full set of mutation operators, taking into
...
SPR, etc.\todo{I'm still not totally certain what we are claiming here; what
  does this contribution do? RESPONSE: we are claiming to have categorized and compared the different mutation operators from the bigger approaches}
We mine
...
We evaluated this approach both on its own, by evaluating the predictive power of the model by analyzing the number of correctly predicted mutation operators and using 10 fold cross
validation;
