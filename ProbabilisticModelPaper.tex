...
mutation operators. \todo{is this redundant with the content in the approach
  section that describes the computation? RESPONSE: There is definitely some overlapping, are you thinking of anything in particular?}
...
Append is the most common single edit mutation operator applied by developers.\todo{In general or in
  the multi-edit case? RESPONSE: I'm not sure what this question refers to but, what it means is that when developers edit code, append is the most common mut operation so a lot of rules will imply append because it is everywhere.} This behavior is
...
replacements model,\todo{It's not obvious to me that association rules for the
  ...
  replacements.  This paragraph/subsection of content may therefore be worth cutting. RESPONSE:
  I am not sure if you are reading the rules how they are meant to be read. These two sets of rules are mined from different bodies of data. We have a bunch of commits, 1) from those commits we take only the replacements, and we create the replacements model. Then we mine that body of data (of replacements only) and we apply Apriori, and we get these association rules. Which are replacements only. These assoc rules say things like: if you replace a block by an if statement AND you also replaced an expression by an If statement THEN the next step would be to replace a block by an if statement. 2) We do that same thing, but instead of replaces only, we do it with mutation operators. The two models are independent, the same with the association rules. Let me know if I can clarify further.} obtaining
...
projects. By contrast with these studies we have considered a full set of mutation operators and a much larger corpus of analysis.  To the best of our knowledge a full set of mutation 
...
Par~\cite{kim2013} describes a study in which the researcher manually look for patterns in the way developers fix their code and describe 10 templates of common behavior to create patches. Our study takes into consideration a superset of these templates and also draws a comparison between these and the statement-edit mutation operators. Our study also provides steps towards accounting for multi-edit source code changes which is a understudied problem common in the majority of these studies.

Synthesis based repair is a family of techniques that uses constraints to build patches following the description of the constraints. These constraints may be specifications created by developers, formal verification, invariants, etc.~\cite{jin11,wei10}. In this study we do not consider synthesis based approaches since the manner in which patches are created (which is the main topic of the study) is considerably different. 
