...
The automatic error fixing process starts by localizing where the fault is, this can be done in different ways as described in Section \ref{background}. After the particular tool 
...
others. In this study we simulate this behavior of human developers in order 
to create patches the closest way to what real developers do, which increases understandability and maintainability.
...
apply these mutation operators to statements in the source code in order to create candidate patches. These and several other techniques use randomized search for patch generation~\cite{arcuri08,bradbury10,Qi14,Weimer13}. 
...
PAR is the product of a study in which researchers examined a large number of human 
...
We created a probabilistic model based on the 500 Java projects in Github 
...
Once we have checked out the 500 Java projects with the most stars, we checkout 
...
validation of the model; the second, a run on an initial example to check how 
...
probability. The acronyms represent the statement kinds detailed in footnote 3}
...
In order to be able to do this, we restructured the code of the tool ``GenProg4Java"\footnote{https://bitbucket.org/clegoues/genprog4java/}, which is the Java version of the 
...
stars. This is in order to generate the largest code base so far to the best 
...
each of the commits studied and created a transaction to be analyzed. A transaction in this context is a row of values that describes which mutation operators were applied in a single commit. We use the 
...
As seen above Confidence is calculated according to its Support, where Support is an indication of how frequently the set of mutation operators (itemset) occurs in the transaction base.
