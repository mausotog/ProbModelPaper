...
maintainability~\cite{Liblit03,Anvik05}. \todo{I'm not sure why you've chosen these two references for this claim...I think Liblit 03 targets C...what's your reasoning? RESPONSE: Liblit talks about how large software systems are deployed with bugs and the resources for improving software is always limited.  RESPONSE: sure, but they don't say anythning about object oriented languages, scalability, or maintainability in particular.  Rephrase the sentence to be broader.}
%
...
from the results of a particular benchmarks. The use of randomized search for patch generation is common among these and other techniques~\cite{arcuri08,bradbury10,Qi14,Weimer13}. \todo{see my slack messages.  AE.  Does.  Not.  Use.  Random.  Search. RESPONSE: I rephrased this sentence especifically to exclude AE. It says the use of randomized search is common. It doesn't say they all have it. Should I rephrase it again to say something like: Some of these approaches apply randomized search? RESPONSE: rephrase it to not include the AE reference.  The sentence says ``the use of randomized search is common'' and then cites a bunch of approaches; the canonical meaning is that those approaches are instances of approaches described in the sentence, that is, those that use randomzied search.}
...
\\
\end{tabular}
  \caption{(Top) PAR fix templates. (Middle) PAR extra templates. (Bottom) SPR transformation schemas.\todo{make the naming consistent---sometimes you use ``Cond'' and sometimes ``Condition''.  And define anything you shorten in the table in the caption.}}
...
\todo{Somewhere in this subsection we should acknowledge the techniques we don't
  ...
  of it to this framework.  Does it fit anywhere?  How?  Why/why not?}
...
question. 
%
To do this we used two existing tools for mapping changes \todo{This used to say
  ``widely used'' which is true of Gumtree but not of the differencing engine in
  QACrashFix so I removed it.  Delete this todo once you've read it.}
...
each of  
them replaces another. For example: What is the observed probability that a For loop 
replaces a While loop? \todo{typeset language constructs like For and While;
  texttt usually works} Given 22 statement types, any of which may replace one
...
\todo{Please add a specific Threats to Validity discussion or subsection.  Look
  up internal, external, and construct validity and describe the threats accordingly.}
