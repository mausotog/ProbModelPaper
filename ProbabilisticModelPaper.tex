...
% in the abstract
\begin{abstract}
Automatic Software Repair has gained a lot of attention in the last several years due to its ability to imitate and succeed in complex processes such as fixing errors in source code; tasks that has been historically very expensive and resource consuming. Most well known state of the art generate-and-validate approaches used for automatic error repair determine which mutation operators to use at a particular fault location by applying an equal distribution to the selection process of these edits. When in reality, mutation operators have very different probabilities of being applied; some mutation operators are far more common than others and are also dependent of the context. Because of this, we have created a probabilistic model that describes the distribution of feasibility when applying each of the different mutation operators. In this study we categorize, compare and evaluate the different mutation operators in the state of the art approaches, we create a model mined from the largest dataset to date, and we create association rules to analyze the context when dealing with multi-edit source code changes, a field that has been understudied in the past and covers the majority of the source code changes in existence. 
\end{abstract}
