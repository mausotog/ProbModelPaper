...
%
We mine association rules from the Mutation operators model as described in Section~\ref{armRes} by analyzing the mutation operator count from the commits studied. We develop
...
operators that frequently happen together, iteratively extending them to larger
...
In terms of
quality assessment, the patches generated behave similarly within
their own groups. In all cases where an approach found multiple
patches, either all the generated patches 
...
We then used our approach using the probabilistic model over the superset of mutation operators, and we broaden our evaluation to all the remaining defects described in Table~\ref{defects4j}; finally, we compared the patches found for all these defects against other state of the art approaches. We do not show all the patches found by the other approaches but just the subset that directly compare to the ones found by our approach.
...
A main takeaway to notice is that the approaches that select what edit to apply 
without a
model, and therefore could benefit from a probabilistic model, performed worse 
than when using a probabilistic approach. Nopol, even though the 5 patches 
created by this tool were able to generalize to the held out test suite, it 
targets a specific kind of bugs and thus isn't as generalizable as our 
approach, which works with course grain mutation operators and is able to generalize to different kinds of defects. Also, a probabilistic model as the one described in this
study, could not be applied to the way patches are constructed in this tool.
Therefore we can clearly see the benefit from applying a probabilistic model,
 in the approaches were a probabilistic model can be applied.
...
\subsubsection{Mutation operator rules} \label{mutOpRules}
...
This is because the Replacements model is used only when the Mutation operators model has chosen a "Replacement'' to
...
Replacements is 1 out of 6859, making association rules for the Replacements corpus an extremely rare case. 
...
rules:\footnote{We will release the full list of mutation operator rules post blind-review.} 
...
To evaluate the effectiveness of the association rules in the context of the automatic program repair process, we first had to remove the section of the corpus where the number of edits performed is less than 3, this is because all the rules created as a result of section~\ref{mutOpRules} require at least two antecedents and one consequent, therefore the patches with less than three edits can not be created by these association rules. This removed 62.83\% of the corpus. On the remaining 37.17\% where association rules can be applied, we performed a 10-fold cross validation process as follows:
...
We categorize them in three different ways: \emph{Fully covered}, \emph{Partially covered}, and \emph{Not covered}.
