...
\todo{Please go into the bibtex and give the different papers by Fan Long more
  informative key names than fan15 and long15 (one is his first name, one is his
  last name) so that I can tell if we're citing the correct thing in a
  particular place by looking at the latex.}
...
Prophet~\cite{fan15}, HDRepair~\cite{xuan16}) use heuristics or heuristically-informed probability distributions to inform the selection of 
which mutation operator
will be applied to construct candidate patches. \todo{A sentence about search strategies
  that is not false.  I blew away the old one because it doesn't have the ``not
  false'' property.  If the sentence says ``many techniques do X'', then the
  references in teh citations at the end of the sentence are assumed to be in
  the class of techniques that do X; it is not the case that we assume that some
  of those references may not do X, even though the word ``Many'' is in the
  sentence being referenced.}
...
single-edit~\cite{Weimer13,Qi15,kim2013}.\todo{there's another reference
  ...
  probably cites it somewhere. RESPONSE: Are you talking about this paper?:
  Adaptive random testing: an illusion of effectiveness? Arcuri and Briand
  ISSTA'11.  RESPONSE RESPONSE: I believe the answer to that is yes.  Please
  cite it.}  Indeed, patches
...
If it doesn't pass all the test cases in 
the test suite used to guide the search, then it will go back to creating more candidate patches and will 
...
operators, and how they travers the search space otherwise. For example, some mechanisms adapt the fitness function to assign higher priority to some candidate patches over others based in heuristics; some others restrict their approach to apply a single edit in the variant creation process; others removes similar patches and prioritizes the test cases according to their likelihood to fail, among others. 
