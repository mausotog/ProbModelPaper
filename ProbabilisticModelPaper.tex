...
If it does not pass all the test cases in 
the test suite used to guide the search, then it\todo{This ``it'' is the it with the unclear antecednet my original todo referred to; it is still unclear.} will go back to creating more candidate patches and will 
...
$ret = z;$ instead of $ret = x;$. \todo{at leaset describe the test cases in
...
in one of the first variants, therefore the number is so low that it does not 
