...
%
We mine association rules from the Mutation operators model as described in Section~\ref{armRes} by transforming each of the mutation operator count from 
each of the commits studied into a \emph{transaction} to be analyzed. We develop
rule sets at difference \emph{Confidence} levels, defined as:

\begin{center}
$conf(X \implies Y) = \dfrac{supp(X \cup Y)}{supp(X)}$ 
\end{center}

Where X and Y are possible items in a transaction (mutation operators, in our
...
be less informative, and because Nopol expresses the time taken to produce each
patch in wall-clock time while we express it in number of variants. 
...
In terms of
quality assessment, the patches generated generally behave similarly within
their own groups. In all cases where an approach found multiple
patches, either all the generated patches 
generalized to the held-out test suite, or none of them did. 
...
\end{tabular}}
  \caption{Comparison between the probabilistic model-based repair and
    other state of the art approaches. The "Found'' column represents the number of patches found for this bug using the different approaches, and the "GTHTS'' column represents the percentage of these patches that Generalized To a Held-out Test Suite, higher is better; because each randomized technique is run several times, some techniques find multiple patches per bug. \label{stateOfTheArtComparison}} 
\end{table*}
...
We then used our approach with the superset of mutation operators and using the probabilistic model, and we broaden our evaluation to all the remaining defects described in Table~\ref{defects4j}; finally, we compared the patches found for all these defects against other state of the art approaches.
...
6.6\% of GenProg's patches generalize; 20\% of TRPAutoRepair's generalize;
15.4\% of PAR's; and and 100\% of the 5 Nopol patches generalize to the held-out test suite.

\todo{Hey, OK, I think this conclusion can be stronger.  Notably, Nopol can't
  fix as many bugs as we can, and fixes slightly different ones, right?  So the
  argument is more like, for heuristic templated approaches, this model is
  important and improves quality of generated patches.  It also fixes bugs that
  Nopol can't, and so is an important improvement over the state of the art.
  Try to improve this paragraph to include that nuance. RESPONSE: So, not 
necessarily we patch more bugs than them, because we are only comparing their 
patches against the ones that we found. We are not reporting the ones that they 
found and we didn't (because we are evaluating our approach and these are the 
bugs we care about. I definitely can make the argument that Nopol can only fix 
a certain kind of bugs and therefore our approach is able to generalize to several different kinds of defects, which I am 
adding)}
...
its predictive power, and by
 running an APR tool using our model on 18 bugs from
...
counterpart.\todo{CLG doesn't understand the sentence right before here.}
%
Overall, we found that automatic program repair appears to benefit
from having a diverse mutation operator pool, increase in search
