...
\ra{1.3}
	\resizebox{\textwidth}{!}{
\begin{tabular}{rcrrcrrcrr}
\toprule 
  &\multicolumn{3}{c}{\textbf{Stmt-Edition}} & \multicolumn{3}{c}{\textbf{Temp-Based}} & \multicolumn{3}{c}{\textbf{All mutations}} \\  
 \hline
...
Single edit source code changes as the ones studied so far in this study are historically the most common subject of analysis in bug fixing tools and studies\todo{cite a bunch of previous work}. 
...
\todo{change these TemplateX by the actual template}
\begin{itemize}
\item Replacements \& Delete $\implies$ Append
\item Delete \& Template1 $\implies$ Append
\item Replacements \& Template6 $\implies$ Append
\item Replacements \& Template2 $\implies$ Append
\item Delete \& Template14 $\implies$ Append
\item Replacements \& Delete \& Template2 $\implies$ Append
\item Replacements \& Template1 $\implies$ Append
\item Replacements \& Delete \& Template6 $\implies$ Append
\item Delete \& Template16 $\implies$ Append
\item Delete \& Template1 \& Template2 $\implies$ Append
\end{itemize}
...
%Minimum metric <confidence>: 0.9
%Number of cycles performed: 20
\todo{Change XrY by the actual statement kinds}
\begin{itemize}
\item 2r13 \& 9r11 $\implies$ 2r11
\item 21r1 $\implies$ 15r3
\item 8r19 $\implies$ 9r13
\item 10r19 $\implies$ 9r21
\item 21r8 $\implies$ 9r21
\item 21r10 $\implies$ 9r21
\item 10r19 $\implies$ 11r19
\item 21r10 $\implies$ 10r19
\item 10r19 $\implies$ 21r10
\item 21r10 $\implies$ 11r19
\end{itemize}
...
\subsection{10 fold cross validation}
\todo{I think this section is unnecessary because, what 10 fold cross validation tells us is that the data is normal, if we divide the data in 10 folds, then each fold behaves similar to the rest. I think the Support measure already tells us how often each rule shows in the overall corpus, which is what 10 fold cross validation analyzes. And more than that, we do are interested in rules that show often in the corpus, but we are much more interested in rules that apply every time that the rule shows (every time that the antecedent shows, the consequent shows as well). So I think, 10 fold cross validation is not an appropriate measure for backing up our data, Confidence is. And we are already optimizing for that.}
