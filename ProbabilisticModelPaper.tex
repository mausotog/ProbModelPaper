...
averaged\todo{or summed, we can show both, rigth? RESPONSE: Sum all the percentages? It would just give us the same number as the average but based on 1000. So if the mean is 87.7, the sum would be 877. I don't see a gain in doing this, but if you think it is useful I can easily do it.} to see how commonly is the training data able to predict the testing 
data.
...
$ret = z;$ instead of $ret = x;$. \todo{at least describe the test cases in
  prose, even if we don't provide them.  How many passing/failing?}

We run our implementation of the GenProg approach targeting Java and using the
equally- and probabilistically-constructed models to select between mutation
operators. \todo{But not the replacement model for replacements?}\todo{Also, in
  the next subsection you say you build the full model on 500 projects.  If
  that's true, what is this model built on?}
...
for time.\todo{Please give details on other genprog settings, as you do later as
  well: timeout, population size, etc.}  We average results of 10 random trials,
...
way you're actually presenting the data?} which means that the tool will disregard all other mutation 
...
equally distributed approach (green bar).\todo{...bars?  It's a table...}
\todo{Also this looks like it succeeds on all seeds even for the equally
  distributed, all-operators approach.  I just want to check: is that true?}
%
...
\center
  \caption{Number of variants it takes to find a patch using different sets of
    ...
    Using Replace. Right) Using Append, Remove and Replace. \todo{Please 
      use text to label units and provide meaning.  What is the
    left, and what is the right, in this table?  Add column headings.}}
...
\ra{1.3}
		\caption{Value on the left of each slot (green): Number of variants
          taken to find a patch in single line bugs. Value on the right of each
          slot (blue): Percentage according to the number of patches that passed
          the second test suite \todo{As above, please don't use color to
            denote meaning and instead use text labels for the associated columns.}\label{tab:singleLineBugs}}
\end{table*}
...
projects in Github with the most stars.\todo{This is confusing with respect to
  the previous section.  What was that model built on?}
...
previous work~\cite{legoues12,kim2013}\todo{does Par say the population size
  ...
  genprog as a search strategy (if that's true).  RESPONSE: Yes, PAR uses 40,
  that is why I mention it in the cite. / Where do I get the generation count
  information? The config file does not say anything, so it uses the default
  one.  RESPONSE REPSONSE: I anticipate that you are able to acertain the
  default used in code you have been contributing to for three years}, we ran 20
seeds per trial, with a timeout of 4 hours for each, in keeping with 
...
Table~\ref{tab:singleLineBugs}.\todo{Why is success rate 0 for Chart 1, given
  that repairs were obviously found?  OH BECAUSE IT'S SUCCESS RATE ON HELD OUT
  TEST SUITE yeah please lable the table properly} In the first row of Table~\ref{tab:singleLineBugs} (Closure \#10) we can see
...
fixed the bug as per the opinion of developers as indicated
in~\cite{kim2013}. \todo{I'm not sure I follow this point.  Try to reexplain?}
...
do not happen very often, but every time that they happen in the analyzed corpus, 
...
Regarding the suitability of our evaluation metrics, the authors evaluate patch quality by running the generated patches through a second test suite created from a human patch, which is to a certain extent a biased measure since we can not guarantee that the human created patch is perfect. Nevertheless, this is a best known practice, especially when compared to the the approach used previously in similar experiments in which a biased human developer would judge whether he/she considered the patch was correct or not.
