...
applied to the source code, which we refer to as ``Mutation operators''; these operators allow for program variations.
There is a broad diversity of these, such as deleting or inserting statements~\cite{legoues12}, applying templates~\cite{kim2013}, transformation schemas~\cite{fan15}, etc.
...
AE~\cite{Weimer13}) use probabilities to select which mutation operator
will be applied to construct candidate patches that are either random or adapted
from the results of a particular benchmarks.
...
human behavior. Different from them, we take this notion of applying a
probabilistic model even earlier in the process, at the time of creating the
patch candidates so all the candidates in the pool are built in a way designed
to fix the error. We also include a more expresisve superset of mutation
...
SPR, etc.\todo{I'm still not totally certain what we are claiming here; what
  does this contribution do? RESPONSE: we are claiming to have categorized and
  compared the different mutation operators from the bigger approaches RESPONSE
  RESPONSE: Rephrase: why is this interesting/useful/contributory to human knowlede?}
We mine
...
We evaluate this approach both on its own, by evaluating the predictive power of the model by analyzing the number of correctly predicted mutation operators and using 10 fold cross
validation;
and as part of a repair algorithm, by applying the model to predict independent
instances. We run an automatic program repair algorithm using the
...
\todo{Suggestion: Rename this to something like ``Generate and validate repair'' or
  ...
  on background in context.  Imperfect, but I think it would be a better
  organization. RESPONSE: I'm confused about this: We talked about having a
  background section that can be skipped by people who know the area. The next
  section is how we categorize the mutation operators, which is a contribution,
  so people shouldn't skip that. I think they are very different sections that
  shouldn't be merged. Also, I did move the background of Association Rule
  mining here. It is the last section of background that talks about support and
  confidence.}

\todo{As mentioned on slack: not all papers need to have the exact same
  structure.  Many benefit from the current type of approach, with background
  serving as background.  I'm arguing that this paper might benefit from an
  alternative approach, as articulated in my original suggestion.}
...
create candidate patches. These and several other techniques use \todo{this
  statement is still false. RESPONSE: Why is this false? They apply randomness
  to create variations selecting mut ops from the search space RESPONSE
  RESPONSE: because AE is deterministic/doesn't use randomness}  randomized 
