...
\subsubsection{Experimental setup} We consider a subset of the Defects4j~\cite{just14}
...
restrict attention to the Defects4J bugs with single-line human patches. 

We perform two types of repair comparisons.  First, we compare the expressive
power (in terms of bugs fixed), variants to repair (a machine- and test
suite-independent proxy for time), and patch quality of different sets of
mutation operators.  That is, we instantiate our tool with three different
operator sets:  (1) Statement-Edit mutations only (2) Template-based
mutations only, and (3) All mutations.

\todo{CLG left off here in her reorg; will return in 30 minutes, after Cody meeting.}
When running this experiment, we set the size for each population to 40 for
consistency with previous work~\cite{legoues12,kim2013}, ran 20 seeds per trial,
10 generations, a timeout of 4 hours for each run, in keeping with
...
To compare the efficacy of different types of mutation operators, we tested
three operation sets:  We average (the number of variants necessary
to create a repair) overall seeds. We use number of variants as a measure for
