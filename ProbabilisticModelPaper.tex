...
programs with minimal human interaction\todo{add citations here, rather than
  later in this paragraph}. One well-known class of repair techniques follows a 
...
leads the program to pass all input test cases. There is a wide a variety of approaches that use generate-and-validate for 
program 
repair~\cite{legoues12,kim2013,Weimer13,fan15,long15,debroy10,perkins09,wei10}\todo{Cut
  this sentence, put the citations up at the previous todo}. 

One reason this task is challenging is the vast search space of possible 
edits that can be applied to the different sections of the source 
code~\cite{long16}. Each such technique defines  a set of possible changes that 
can be applied to the source code, allowing for program variations; this 
...
Given a potentially-buggy program location, generate-and-validate repair must
select between such possible schemas.  
...
AE~\cite{Weimer13}, etc. use probabilities to select which mutation operator
will be applied to construct candidate patches that are either random or adapted
from the results of a particular benchmarks, which may lead to plausible patches
that aren't similar to what a developer would do, and therefore harder to
understand, less efficient and less maintainable~\cite{Qi15}.\todo{the Qi15
  reference doesn't talk about understandability, efficiency, nor
  maintainability...the Par paper might be a better reference, and/or maybe the
  Prophet paper?  Again,
  I'm wary of making claims about plausibility versus correctness, because I
  don't think we evaluate it.} 

\todo{I would move this entire paragraph out of intro and into the background subsection on G-and-V
  repair.} The automatic error fixing process starts by localizing the defect,
this can be achieved in several different ways (eg, Ochiai, Jaccard, SBI,
Tarantula, etc\todo{also whenever you mention a prior technqiue, as you do in
  this list, refer to the paper that proposes it.  This is always true.}). It then goes through a process in which it analyzes which mutation operators can
...
others.\todo{an example highlighting the intutition, even just in prose/without
  code, might help.} In this paper we study and then simulate the behavior of human developers in order 
...
%
We also handle, compare and contrast a full set of mutation operators, taking into
account the full set of mutation operators currently used in the most widely
used state of the art approaches, such as GenProg, Prophet, PAR, AE, RSRepair,
SPR, etc.\todo{I'm still not totally certain what we are claiming here; what
  does this contribution do?}
We mine
the 500 most popular Github projects to produce a model of the probability of
...
to fix bugs in real world projects. We therefore more closely
simulate the manner in which human programmers patch their code, which has 
being evaluated to create patches that are easier to understand by
humans~\cite{kim2013}\todo{this claim should go up above.} and 
therefore it increases the maintainability of the patches, following a similar
intuition to Prophet\cite{long15} which ranks the created candidate patches
according to their likelihood to fix the bug based on a model constructed by
human behavior.\todo{ the problem with this sentence is that it's not making it
  clear how we're different/better or what the limitation of Prophet is.  why do
  we need to exist, if they already do?  Move this claim above to where you
  introduce the idea of selecting between mutation operators and highlight the
  deficiency with it that motivates this study.}

We evaluated this approach both on its own, by using 10 fold cross
validation,\todo{this sentence still needs to say what metrics we're using, per
  our conversation on Slack.}
and as part of a repair algorithm, by applying the model to predict independent
instances. We use 10 fold cross validation to structure the evaluation of
predictive power to increase confidence in the generalizability of the
results. Afterwards, we run the an automatic program repair algorithm using the
probabilistic model with a subset of the mutation  
...
\todo{Suggestion: Rename this to something like ``Generate and validate repair'' or
  something similar, add subsections, and combine it with the content currently
  in the next section.  I think I asked you to move the association rule mining
  background (back?) to the section on association rule mining as a subsection
  on background in context.  Imperfect, but I think it would be a better organization.}
...
creating candidate 
patches is by applying low level\todo{wrong phrasing; the key behind these
  techniques is that statement-level mutations aren't low-level but are instead
  fairly coarse.  Rephrase; this is misleading.} mutation operators (such as append, delete, or 
...
some action to be carried out in a programming language.\todo{Careful, this
  isn't true.  Statements are a grammar nonterminal in C, referring to a
  specific language construct.  They correspond to language constructs at a
  similar granularity level in Java.  Say something like that, with an example,
  like ``including 
if-then-else or instruction statements.''} These tools are able to 
apply these mutation operators to statements in the source code to 
create candidate patches. These and several other techniques use \todo{this
  statement is still false}  randomized 
...
\todo{the formatting on this doesn't work, several ways; left-align text, and
  it's too wide.  I think a proper figure might work better than an in-line
  table like this.}
...
between two versions of code: Gumtree~\cite{falleri14} and
QACrashFix~\cite{gao15}.\todo{As mentioned in my previous notes, QACrashFix is a
  bug fixing tool, not a differencing tool, so please be explicit about the
  element you'r eusing here.}
...
being manipulated is an If statement, we then  ask if the action being performed
is an Insertion of a node. After we know that it is an insertion of an If
statement, we then ask if the first condition within the If statement is an
InfixExpression. An InfixExpression as defined by the Eclipse JDT API
...
\\
\\  
Afterwards, we ask if the Expression following the InfixOperator is a
\todo{Find a way to typeset language constructs differently from regular text.}
NullLiteral. Once we know all this information we know that a null checker
template was applied to the code, since we know that the developer added an If
statement, whose conditional is conformed by an Expression being compared to a
NullLiteral, so we count it as an instance of this template. 
...
a given model.  Our probabilistic model is two-level: the 
\textit{Mutation operator probabilistic model} and 
the \textit{Replacements probabilistic model}.

\todo{What is the motivation between a two-level model?}
\todo{I think these could be labelled paragraphs rather than subsubsections.}
...
We analyzed the 22 different kinds of statement detailed by Eclipse JDT\todo{add
  a footnote linking to the official JDT rather than listing all statements in
  the footnote.} as
...
\todo{Maybe this should be part of the preceding section, as a subsection?  Also
  see my note on the background}

Single-mutation or -edit approaches are historically the most 
...
\todo{Restructure; No single subsections.  Any single subjsection should be
  folded into a section without subsections, or divided further.}
...
technique.  We first \todo{What are the metrics for the independent model
  evaluation?} (Section~\ref{sec:generalize}).  Next, we asses a case study
...
\todo{Is there a way to separate out the association rule content from the
  association rule results, and put the latter in this section?}
...
\todo{I'd make this a subsection of the previous section.}
