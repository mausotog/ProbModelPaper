...
Syntactic or heuristic approaches~\cite{examples} construct candidate patches out of one or
more Abstract Syntax Tree (AST)-level program modifications, applied at
at likely-faulty program locations. 
To construct a new modification, given a likely-faulty statement, a mutation operator is selected from a
predefined set of such operators.  This process typically excludes those that are not applicable to the
given location or that would lead to a compilation failure. For
...
first line of a constructor.  Most mutation operators (with the exception of deletion) require ``fix'' code to fill in
one or more ``holes'' (\emph{insert}, for example, requires code to insert at a
location); this code is typically selected from elsewhere in the same file,
program, or module.  

Semantics-guided generate-and-validate APR~\cite{examples,esp,nopol} differ from
the syntactic approaches described above in the types of modifications they
consider and how they are instantiated.  

Regardless of how the mutations are selec
...
approaches assign
a higher priority to some candidate patches over others based on heuristics~\cite{long16proph}; others are explicitly single-edit approaches~\cite{Qi13TrpAutoR,Weimer13}; others use various other
random search heuristics, like a random walk~\cite{debroy10} or genetic
programming~\cite{kim2013,legoues12,xuan16}. 


The quality of the patches created
by APR techniques is commonly evaluated by using a held-out test suite (a test
suite that is from the one used to guide the search, but it outlines the same
expected behavior). If the generated patch is able to pass all the test cases
...
\subsection{Generate-and0validate mutation operators} 
