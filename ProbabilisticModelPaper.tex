...
maintenance costs by reducing the huma effort required to localize and fix
bugs. Well-known state-of-the-art generate-and-validate 
APR approaches select between various mutation operators
to constructing candidate patches, and do so either according to a probability
distribution of \todo{Mau can you please try to rephrase this clause?  I'm not
  sure what you mean}  adapted from
the results of a particular benchmark.  This may reduce lead to patches that are harder to
understand and less maintainable.\todo{OK, we're not evaluating
  understandability/maintainability in this work, so I would add something in
  here about correctness or expressive power instead (since that's what we do
  evaluate)}. In engineering practice, \todo{Phrasing issue: ``mutation operators'' has a
  particular meaning, especially in the context of mutation testing research, so
  please avoid overloading/reusing it like this.  I think you really mean
  something like ``in practice, human bug fixers take multiple strategies to fix
  bugs, and the transformations they select between are not uniformly
  likely...'' or something like that.  Maybe you mean ``transformations'' or
  ``templates'' or something, but mutation operators is a bit confusing.  Please
  rephrase.}  mutation operators have  
...
mine FIXME bug-fixing commits from FIXME projects to create a probabilistic model describing the distribution of 
feasibility\todo{feasibility?  I don't think this is the correct word; rephrase,
  and then adjust the rest of the sentence accordingly.} when applying each of the different mutation operators. 
Wtudy we categorize, compare and evaluate the different mutation operators in 
...
in the past and covers the majority of real source code changes. \todo{one
  sentence summary indicating nature of evaluation/results, even if you don't
  have numbers.}
\end{abstract}
...
validation of the model; the second, a run on an case study to check how 
...
\hline
\textbf{Seed} & \textbf{0} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} & \textbf{9}  \\
\hline
\textbf{Prob Model} & 8 & 20 & 23 & 42 & 4 & 11 & 0 & 29 & 80 & 0 \\

\textbf{Eq Dist} & 8 & 66 & 23 & 42 & 4 & 11 & 2 & 38 & 81 & 139 \\
\hline
\end{tabular}
\\
\\
